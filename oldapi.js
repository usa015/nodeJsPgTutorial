...
app.get("/users/search/city", urlencodedParser, (req, res) => {
  ...
  if (srchStr)
    client.query(
      "select DISTINCT geo_id, geographic_area_name from city_master where geographic_area_name ilike '" +
        srchStr +
      ...
      (err, result) => {
        if (!err) {
          ...
          for (let index = 0; index < result.rows.length; index++) {
            myobj.data.push({
              name: result.rows[index].geographic_area_name,
              id: result.rows[index].geo_id,
            });
          }
...
app.get("/users/geographicAreaName", urlencodedParser, (req, res) => {
  ...
  const myobj = { data: [] };
  client.query(
    "select geo_id, geographic_area_name from city_master",
    (err, result) => {
      if (!err) {
        for (let index = 0; index < result.rows.length; index++) {
          myobj.data.push({
...
app.post("/search", urlencodedParser, (req, res) => {
  ...
  if (y) {
    labelDescQuery =
      "select use_case_group, use_case_group_desc, priority_label from use_case_master where use_case_group in (" +
      usecaselabel +
      ") ORDER BY priority_label";
  } else {
    labelDescQuery =
      "select use_case_group, use_case_group_desc, priority_label from use_case_master ORDER BY priority_label";
  }
  ...
  geographicQuery =
    "select geo_id,ten_year_pop_growth_rate, home_p_to_income, median_income from city_view_2020_updated where geo_id in (" +
    x +
    ") and year = 2020 order by geo_id";
        ...
        for (let j = 0; j < result.rows.length; j++) {
          if (result.rows[j].geo_id == myArray[index]) {
            ...
            if (result.rows[j].median_income) {
              medianIncomeCalculation =
                parseInt(result.rows[j].median_income) / 1000;
              medianIncomeCalculation = parseFloat(
                medianIncomeCalculation
              ).toFixed(1);
            ...
            if (result.rows[j].home_p_to_income) {
              homeptoincomeCalculation = parseFloat(
                result.rows[j].home_p_to_income
              ).toFixed(1);
            ...
            if (result.rows[j].ten_year_pop_growth_rate) {
              tenYearPopGrowthRate = parseFloat(
                result.rows[j].ten_year_pop_growth_rate
              ).toFixed(1);
  ...
  let query =
    "SELECT geo_id, OR_O, OR_BV, OR_MA, OR_CWYK, OR_YP, OR_R, OR_L FROM index_final_scores_new_1 WHERE geo_id  IN (" +
    x +
    ") and publication_year=2020";
...
app.get("/users/usecase", urlencodedParser, (req, res) => {
  ...
  const obj = { data: [] };
  client.query(
    "select * from use_case_master ORDER BY priority_label",
    (err, result) => {
      if (!err) {
        for (let index = 0; index < result.rows.length; index++) {
          obj.data.push({
            name: result.rows[index].use_case_group_desc,
            id: result.rows[index].use_case_id,
            code: result.rows[index].use_case_group,
            priority: result.rows[index].priority_label,
          });
        }
